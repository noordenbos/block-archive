...
2. A screenshot of the actual inventory UI running in a fresh temporary archive, with four reviewed tissue-side photographs and `DEMO` identifiers. The fixed selection tray is hidden for the cropped panel.
3. Two actual app captures: a real tissue-side crop with demonstration retained-piece outlines and dashed scoring lines, followed by that retained piece alongside three additional demonstration tissue polygons on a recipient slide. The outlines and placements are illustrative, not an actual experiment plan.
...
The archive panel also uses `tissue-block-2.jpg`, `tissue-block-3.jpg`, and `tissue-block-4.jpg`, reviewed for visible identifiers and embedded metadata. Public records use DEMO IDs. The green selection outline on slide piece 1 links it to the scored block shown alongside it.
