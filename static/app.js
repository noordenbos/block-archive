...
async function selectBlock(id, updateURL=true){
  ...
  try{const record=await api('/api/v1/blocks/'+encodeURIComponent(id));if(generation!==selectionGeneration)return;
    ...
    if(updateURL)history.pushState({},'', '/?block='+encodeURIComponent(record.block_code));
...
function renderRecord(){
  ...
  loadCaseBlocks(b);
  $('#copyLink').onclick=async()=>{try{await navigator.clipboard.writeText(location.origin+'/?block='+encodeURIComponent(b.block_code));message('Block record link copied.');}catch{message('Use the address bar to copy this block’s link.',true);}};
