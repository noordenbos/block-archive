...
function render(){const item=selected;
  ...
  if(item.block_id){$('#openBlock').onclick=async()=>{try{const block=await api('/api/v1/blocks/'+item.block_id);location.href='/?block='+encodeURIComponent(block.block_code);}catch(error){message(error.message,true);}};return;}
