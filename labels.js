...
async function generate(){
  ...
  try{const r=await fetch('/api/labels',{method:'POST',headers:{'X-Spatial-Local':'1'},body:currentCSV});if(!r.ok)throw Error('Cannot generate labels. Use a one-column CSV (1–2000 IDs).');
...
el('pdf').onclick=async()=>{const r=await fetch('/api/labels.pdf',{method:'POST',headers:{'X-Spatial-Local':'1'},body:currentCSV});if(!r.ok){el('status').textContent='PDF generation failed.';return}const a=document.createElement('a'),url=URL.createObjectURL(await r.blob());a.href=url;a.download='spatial-id-qr-labels.pdf';a.click();setTimeout(()=>URL.revokeObjectURL(url),10000)};
