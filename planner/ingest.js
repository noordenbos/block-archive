...
renderSlides=function(){
  ...
  if(p&&$('#angle')){$('#angle').oninput=e=>{if(e.target.value!=='')rotate(Number(e.target.value))};$('#angle').onchange=()=>save();}
...
ingestFolder=async function(e){
  ...
  for(const [id,pair] of groups){
    ...
    const decision=SideDetection.chooseSides(pair);
    if(!reason&&!decision)reason='Upper-rim lettering is insufficient, appears on both views, or gives conflicting side evidence; images retained for manual assignment.';
    ...
    createPairedBlock(id,pair[decision.tissue],pair[decision.identifier]);
  }
