...
from .side_detection import printing_evidence
...
def analyze(raw):
    ...
    for view in (im,canonical,canonical[700:1000,200:750] if page_h is not None else im):
        for factor in (1,.75,.5,1.5):
            ...
            if values: break
        if values: break
    calibration=None; role='unknown'; role_score=0.; transform=None; label_features=0; printing=None
    if page_h is not None:
        ...
        printing=printing_evidence(zone)
        role='identifier' if label_features>=12 or (printing['score']>=5 and printing['heightVariation'] is not None and printing['heightVariation']<=.4) else 'unknown'
    ...
    return {'hash':hashlib.sha256(raw).hexdigest(),'qrValues':values,'markerIds':sorted(markers),'registration':registration,
            'rotationDegrees':angle,'role':role,'gridFeatures':role_score,'labelFeatures':label_features,'printingEvidence':printing,'calibration':calibration,
